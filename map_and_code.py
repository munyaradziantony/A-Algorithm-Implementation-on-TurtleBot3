    ...
    def a_star(self, start: Point, goal: GoalPt):
        ...
        while self.queue:
            ...
            for dx, dy, dtheta, cost in valid_actions:
                ...
                if 0 <= next_node.x < WIDTH and 0 <= next_node.y < HEIGHT and not is_inside_obstacles(int(next_node.x),
                                                                                                      int(next_node.y)):
                    if next_node.visited == False or next_node.c2c > node.c2c + cost:
                        next_node.c2c = node.c2c + cost
                        # Updating total cost to ctoc with heuristic
                        next_node.total_cost = next_node.c2c + heuristic(Point(next_node.x, next_node.y, 0),
                                                                         self.search_goal)
...
def get_radius_and_clearance():
    ...
    while True:
        try:
            ...
            if not (0 < radius <= 5):
                print("Warning: Robot radius out of bounds. Using default radius (5 mm)")
                radius = 5
            break
        except ValueError:
            print("Warning: Robot radius out of bounds. Using default radius (5 mm)")
...
def get_valid_start_coordinates(radius, clearance):
    ...
    while True:
        try:
            x_s, y_s = map(int, input("Start (x y ) e.g 10 10: ").split())
            if not (0 <= x_s < WIDTH and 0 <= y_s < HEIGHT):
                print("Error: Start point out of bounds.")
                continue
            if is_inside_obstacles(x_s, HEIGHT - y_s):
...
def get_valid_goal_coordinates(radius, clearance):
    ...
    while True:
        try:
            x_g, y_g = map(int, input("Goal (x y) e.g 550 230 : ").split())
            if not (0 <= x_g < WIDTH and 0 <= y_g < HEIGHT):
                print("Error: Goal point out of bounds.")
                continue
            if is_inside_obstacles(x_g, HEIGHT - y_g):
