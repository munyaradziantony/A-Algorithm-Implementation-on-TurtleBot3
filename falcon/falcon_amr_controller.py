...
class RobotAStarPlannerNode(Node):
    def __init__(self):
        ...
        int_start = [int(start[0]*100), int(start[1]*100), start[2]]
        ...
        path,ret = plan_path(tuple(int_start),tuple(int_end),robot_radius,clearance,delta_time,goal_threshold,wheel_radius,wheel_distance,rpms[0],rpms[1])
        self.get_logger().info(f'Path length {len(path)} return val {ret}')
